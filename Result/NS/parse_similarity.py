#!/usr/bin/env python3
"""
Parse "Most similar words" sections from NS result files and create CSV for similarity heatmap
"""
...
def parse_similarity_section(content: str, model_type: str) -> List[Dict]:
    ...
    # Find where this section ends (next section marker like "Word pair similarities:" or evaluation end)
    section_content = content[start_idx:]
    
    # Find the end of this section - look for "Word pair similarities:" or "Evaluation results saved"
    end_markers = [
    ...
    for marker in end_markers:
        marker_idx = section_content.find(marker, len(start_marker))
    ...
    for line in lines:
        ...
        # Skip the header line "Most similar words:"
        if line_stripped == "Most similar words:":
            continue
        
        # Check if this is a word header (word: on its own line, no leading spaces)
        word_match = re.match(r'^(\w+):\s*$', line_stripped)
        if word_match:
            current_word = word_match.group(1)
            continue
        
        # Check if this is a similarity entry (leading spaces, then word: 0.1234)
        # Format: "  word: 0.1234" or "    word: 0.1234"
        if current_word:
            ...
            if similarity_match:
                ...
                # If we have a current_word but this line doesn't match similarity format,
                # and it's not empty, it might be a new section - stop parsing
...
def parse_all_files(base_dir: str) -> List[Dict]:
    """Parse all result files in the directory - NS files are directly in the directory"""
...
def main():
    ...
    similarity_csv = base_dir / 'NS_similarity_long.csv'
    matrix_csv = base_dir / 'NS_similarity_matrix.csv'
    word_focused_csv = base_dir / 'NS_similarity_by_word.csv'
